 ...
 * *					calculation had a bunch of protectediables without $ symbols.
 * *					Fixed an error in convertLLtoTM() method, The $this-> was
 * *					missing in front of a couple of protectediables. Thanks to Bob
...
//		$y -= 10000000.0;	// Uncomment line to make LOCAL coordinates return southern hemesphere Lat/Long
		...
		$R1 = $this->a * (1 - $this->e2) / pow( 1 - $this->e2 * sin( $phi1Rad ) * sin( $phi1Rad ) ,
						1.5 );
		$D = $x / ($N1 * $k0);
...
//
	function configLambertProjection( $falseEasting , $falseNorthing ,
			$longOfOrigin , $latOfOrigin , $firstStdParallel , $secondStdParallel )
	{
...
//
	function convertLLtoLCC()
	{
		...
		$t1 = tan( (pi() / 4) - ($phi1 / 2) ) / pow( ( ( 1 - $e * sin( $phi1 ) ) / ( 1 + $e * sin( $phi1 ) ) ) ,
						$e / 2 );
		$t2 = tan( (pi() / 4) - ($phi2 / 2) ) / pow( ( ( 1 - $e * sin( $phi2 ) ) / ( 1 + $e * sin( $phi2 ) ) ) ,
						$e / 2 );
		$to = tan( (pi() / 4) - ($phio / 2) ) / pow( ( ( 1 - $e * sin( $phio ) ) / ( 1 + $e * sin( $phio ) ) ) ,
						$e / 2 );
		$t = tan( (pi() / 4) - ($phi / 2) ) / pow( ( ( 1 - $e * sin( $phi ) ) / ( 1 + $e * sin( $phi ) ) ) ,
						$e / 2 );
...
//
	function convertLCCtoLL()
	{
		...
		$phi0 = (pi() / 2) - 2 * atan( $t_ );
		$phi1 = (pi() / 2) - 2 * atan( $t_ * pow( ((1 - $e * sin( $phi0 )) / (1 + $e * sin( $phi0 )) ) ,
								$e / 2 ) );
		$phi2 = (pi() / 2) - 2 * atan( $t_ * pow( ((1 - $e * sin( $phi1 )) / (1 + $e * sin( $phi1 )) ) ,
								$e / 2 ) );
		$phi = (pi() / 2) - 2 * atan( $t_ * pow( ((1 - $e * sin( $phi2 )) / (1 + $e * sin( $phi2 )) ) ,
								$e / 2 ) );
